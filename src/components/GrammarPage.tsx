          ...
          >
            {grammarData.map((grammar) => (
              <AccordionItem
                key={grammar.id}
                value={grammar.id}
                className="bg-white rounded-[24px] shadow-lg overflow-hidden border-none"
              >
                <AccordionTrigger className="px-6 py-5 hover:bg-gradient-to-r hover:from-[#D8C8FF]/10 hover:to-[#C7FFF1]/10 transition-all [&[data-state=open]]:bg-gradient-to-r [&[data-state=open]]:from-[#D8C8FF]/20 [&[data-state=open]]:to-[#C7FFF1]/20">
                  <div className="flex items-center gap-4 flex-1 text-left">
                    ...
                    <div className="flex-1">
                      <h3 className="text-xl text-gray-800">{grammar.title}</h3>
                    </div>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="px-6 pb-6">
                  <div className="space-y-6 pt-4">
                    {/* Structure */}
                    <div className="bg-gradient-to-r from-[#FFF6E9] to-[#FFC7EA]/10 rounded-[16px] p-4">
                      <h4 className="text-sm text-gray-600 mb-2">Cấu trúc:</h4>
                      <p className="text-lg text-gray-800">{grammar.structure}</p>
                    </div>

                    {/* Meaning */}
                    <div className="bg-gradient-to-r from-[#D8C8FF]/10 to-[#C7FFF1]/10 rounded-[16px] p-4">
                      <h4 className="text-sm text-gray-600 mb-2">Ý nghĩa:</h4>
                      <p className="text-lg text-gray-800">{grammar.meaning}</p>
                    </div>

                    {/* Examples */}
                    <div>
                      <h4 className="text-sm text-gray-600 mb-3">Ví dụ:</h4>
                      <div className="space-y-3">
                        {grammar.examples.map((example, idx) => (
                          <div
                            key={idx}
                            className="bg-white border-2 border-[#FFC7EA]/30 rounded-[16px] p-4 hover:border-[#FFC7EA] transition-colors"
                          >
